...
/**
    ...
    {
        list($response) = $this->trackingsGetWithHttpInfo($authorization, $tracking_number, $contentType);
        return $response;
    }
     ...
     */
    public function trackingsGetAsync($authorization, $tracking_number, string $contentType = self::contentTypes['trackingsGet'][0])
    {
        return $this->trackingsGetAsyncWithHttpInfo($authorization, $tracking_number, $contentType)
            ->then(
                function ($response) {
                    return $response[0];
                }
    ...
    /**
     * Operation trackingsGetAsyncWithHttpInfo
     *
     * Get Tracking Details
     *
     * @param  string $authorization Authorize your API calls with an Arta API token (required)
    ...
    /**
     * Create request for operation 'trackingsGet'
     *
     * @param  string $authorization Authorize your API calls with an Arta API token (required)
     * @param  string $tracking_number tracking_number parameter (required)
     * @param  string $contentType The value for the Content-Type header. Check self::contentTypes['trackingsGet'] to see the possible values for this operation
     ...
     */
    public function trackingsGetRequest($authorization, $tracking_number, string $contentType = self::contentTypes['trackingsGet'][0])
    {

        // verify the required parameter 'authorization' is set
        if ($authorization === null || (is_array($authorization) && count($authorization) === 0)) {
            throw new \InvalidArgumentException(
                'Missing the required parameter $authorization when calling trackingsGet'
            );
        }
        ...
        if ($tracking_number === null || (is_array($tracking_number) && count($tracking_number) === 0)) {
            throw new \InvalidArgumentException(
                'Missing the required parameter $tracking_number when calling trackingsGet'
            );
        }


        $resourcePath = '/trackings/{tracking_number}';
        $formParams = [];
        $queryParams = [];
        $headerParams = [];
        $httpBody = '';
